...
/******/ 	var hotCurrentHash = "69055fd96aae095fb003"; // eslint-disable-line no-unused-vars
...
var namespace_1 = __webpack_require__("./components/controller/namespace.ts");
var Controller = /** @class */ (function () {
    function Controller(model, view, callbacks) {
        this.model = model;
        this.view = view;
        this.callbacks = __assign({}, namespace_1.initialOptions);
        this.customEvents = __assign({}, namespace_1.initialCustomEvents);
        this.init(callbacks);
    }
    Controller.prototype.init = function (callbacks) {
    ...
    Controller.prototype.setCallbacks = function (callbacks) {
    ...
    Controller.prototype.attachViewEvents = function () {
        var _this = this;
        var model = this.model;
        var view = this.view;
        view.events.fromChanged.attach(function (realValue) {
            model.calcFromWithStep(_this.convertToReal(realValue));
        });
        view.events.toChanged.attach(function (realValue) {
            model.calcToWithStep(_this.convertToReal(realValue));
        });
        view.events.slideStart.attach(function (handle) {
            _this.emitEvent(_this.customEvents.start, 'vanillastart', handle, _this.callbacks.onStart, _this.getModelViewData());
        });
        view.events.slideEnd.attach(function (handle) {
            _this.emitEvent(_this.customEvents.end, 'vanillaend', handle, _this.callbacks.onEnd, _this.getModelViewData());
        });
    };
    Controller.prototype.attachModelEvents = function () {
        var _this = this;
        var model = this.model;
        var view = this.view;
        model.events.fromChanged.attach(function (fromValue) {
            view.calcFrom(_this.convertToPercent(fromValue));
            _this.emitEvent(_this.customEvents.slide, 'vanillaslide', view.nodesData.from, _this.callbacks.onSlide, _this.getModelViewData());
        });
        model.events.toChanged.attach(function (toValue) {
            view.calcTo(_this.convertToPercent(toValue));
            _this.emitEvent(_this.customEvents.slide, 'vanillaslide', view.nodesData.to, _this.callbacks.onSlide, _this.getModelViewData());
        });
    };
    Controller.prototype.convertToPercent = function (realValue) {
        var modelData = this.model.data;
        var range = modelData.max - modelData.min;
        return Number(((realValue - modelData.min) * 100 / range).toFixed(10));
    };
    Controller.prototype.convertToReal = function (pixels) {
        var modelData = this.model.data;
        var viewNodes = this.view.nodesData;
        var range = modelData.max - modelData.min;
    ...
    Controller.prototype.emitEvent = function (customEvent, eventName, eventSrc, clientCallback, eventData) {
        customEvent = this.createSliderEvent(eventName, eventData);
        this.callClientCallback(customEvent, __assign({}, this.model.data, this.view.data), clientCallback);
        eventSrc.dispatchEvent(customEvent);
    };
    Controller.prototype.callClientCallback = function (event, data, callback) {
        if (callback) {
            callback(event, data);
        }
    };
    Controller.prototype.createSliderEvent = function (eventName, data) {
    ...
    Controller.prototype.getModelViewData = function () {
        return __assign({}, this.model.data, this.view.data);
    };
    return Controller;
}());
...
/***/ "./components/controller/namespace.ts":
/***/ (function(module, exports, __webpack_require__) {
...
var namespace_1 = __webpack_require__("./components/model/namespace.ts");
var decko_1 = __webpack_require__("../node_modules/decko/dist/decko.js");
var Model = /** @class */ (function () {
    function Model() {
        ...
        this.options = __assign({}, namespace_1.initialOptions);
    }
    ...
    Model.prototype.calcFromWithStep = function (realValue) {
        var opt = this.options;
        if (opt.fromFixed) {
            return;
        }
        var valueWithStep = Math.round((realValue - opt.min) / opt.step);
        var valueOffset = valueWithStep * opt.step + opt.min;
        var valueInDiapason = opt.type ? this.correctDiapason(valueOffset, opt.min, opt.to) : this.correctDiapason(valueOffset, opt.min, opt.max);
        if (valueInDiapason !== opt.from) {
            opt.from = valueInDiapason;
            this.modelEvents.fromChanged.notify(opt.from);
        }
    };
    Model.prototype.calcToWithStep = function (realValue) {
        var opt = this.options;
        ...
        var valueWithStep = Math.round((realValue - opt.min) / opt.step);
        var valueOffset = valueWithStep * opt.step + opt.min;
        var valueInDiapason = this.correctDiapason(valueOffset, opt.from, opt.max);
    ...
    Model.prototype.correctDiapason = function (value, min, max) {
        if (min === void 0) { min = this.options.min; }
    ...
    __decorate([
        decko_1.bind
    ], Model.prototype, "calcFromWithStep", null);
    __decorate([
        decko_1.bind
    ], Model.prototype, "calcToWithStep", null);
    return Model;
}());
...
var decko_1 = __webpack_require__("../node_modules/decko/dist/decko.js");
var observer_1 = __webpack_require__("./components/observer/observer.ts");
var namespace_1 = __webpack_require__("./components/view/namespace.ts");
var View = /** @class */ (function () {
    ...
    Object.defineProperty(View.prototype, "events", {
        get: function () { return this.viewEvents; },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(View.prototype, "nodesData", {
        get: function () { return this.nodes; },
        enumerable: true,
        configurable: true
    });
    ...
    View.prototype.update = function (viewOptions) {
    ...
    View.prototype.calcFrom = function (from) {
        ...
        var base = opt.orientation === 'vertical' ? 'bottom' : 'left';
    ...
    View.prototype.calcRange = function () {
        ...
        var opt = this.options;
        var nodes = this.nodes;
        var baseStart = opt.orientation === 'vertical' ? 'bottom' : 'left';
        var baseEnd = opt.orientation === 'vertical' ? 'top' : 'right';
        var handle = opt.type === 'double' ? 'to' : 'from';
        nodes.range.style[baseStart] = opt.type === 'from-start' ? '0' : nodes.from.style[baseStart];
        nodes.range.style[baseEnd] = opt.type === 'from-end' ? '0' : 100 - parseFloat(nodes[handle].style[baseStart]) + "%";
    };
    ...
    View.prototype.mouseDown = function (event) {
        var nodes = this.nodes;
        var coord = this.getRelativeCoord(event);
    ...
    View.prototype.mouseMove = function (event) {
        var nodes = this.nodes;
        var coord = this.getRelativeCoord(event);
        var eventForNotify = this.handle === nodes.from ? 'fromChanged' : 'toChanged';
    ...
    View.prototype.chooseHandle = function (mouseCoord) {
        var opt = this.options;
        var nodes = this.nodes;
        if (this.handle) {
            return this.handle;
        ...
        var fromCoord = this.getHandleCoord(nodes.from);
        var toCoord = this.getHandleCoord(nodes.to);
        if (opt.orientation === 'horizontal' && (mouseCoord < fromCoord)) {
            return nodes.from;
        }
        if (opt.orientation === 'horizontal' && (mouseCoord > toCoord)) {
            return nodes.to;
        }
        if (opt.orientation === 'vertical' && (mouseCoord > fromCoord)) {
            return nodes.from;
        }
        if (opt.orientation === 'vertical' && (mouseCoord < toCoord)) {
            return nodes.to;
        }
        return Math.abs(fromCoord - mouseCoord) < Math.abs(toCoord - mouseCoord) ? nodes.from : nodes.to;
    };
    View.prototype.getRelativeCoord = function (event) {
        var nodes = this.nodes;
        var axis = this.options.orientation === 'horizontal' ? 'pageX' : 'pageY';
        var offset = this.options.orientation === 'horizontal' ? 'offsetLeft' : 'offsetTop';
        return event[axis] - nodes.track[offset];
    };
    View.prototype.getHandleCoord = function (handle) {
        var offset = this.options.orientation === 'horizontal' ? 'offsetLeft' : 'offsetRight';
        var size = this.options.orientation === 'horizontal' ? 'offsetWidth' : 'offsetHeight';
    ...
    __decorate([
        decko_1.bind
    ], View.prototype, "calcFrom", null);
    __decorate([
        decko_1.bind
    ], View.prototype, "calcTo", null);
    __decorate([
        decko_1.bind
    ], View.prototype, "mouseDown", null);
    __decorate([
        decko_1.bind
    ], View.prototype, "mouseMove", null);
    __decorate([
        decko_1.bind
    ], View.prototype, "mouseUp", null);
    return View;
}());
...
(0, _jquery2.default)(document).ready(function () {
  var $create = (0, _jquery2.default)('.js-callback_create'),
      $slide = (0, _jquery2.default)('.js-callback_slide'),
      $start = (0, _jquery2.default)('.js-callback_start'),
      $end = (0, _jquery2.default)('.js-callback_end'),
      $update = (0, _jquery2.default)('.js-callback_update'),
      $from = (0, _jquery2.default)('input[name="from"]'),
      $to = (0, _jquery2.default)('input[name="to"]'),
      $min = (0, _jquery2.default)('input[name="min"]'),
      $max = (0, _jquery2.default)('input[name="max"]'),
      $type = (0, _jquery2.default)('input[name="type"]'),
      $orientation = (0, _jquery2.default)('input[name="orientation"]'),
      $step = (0, _jquery2.default)('input[name="step"]'),
      $fromFixed = (0, _jquery2.default)('input[name="fixed-from"]'),
      $toFixed = (0, _jquery2.default)('input[name="fixed-to"]');
  var onCreateTimeId = void 0,
      onStartTimeId = void 0,
      onSlideTimeId = void 0,
      onEndTimeId = void 0,
      onUpdateTimeId = void 0;

  var remove = function remove() {
    (0, _jquery2.default)('.callback').removeClass('active');
  };
  ...
  var slider = new _slider2.default('.js-slider', {
    ...
    onSlide: function onSlide(event, data) {
      $from.val(data.from);
      $to.val(data.to);
    ...
    onEnd: function onEnd(event, data) {
      $from.val(data.from);
      $to.val(data.to);
      $end.addClass('active');
...
var namespace_1 = __webpack_require__("./components/model/namespace.ts");
var namespace_2 = __webpack_require__("./components/view/namespace.ts");
var namespace_3 = __webpack_require__("./components/controller/namespace.ts");
var SliderConstructor = /** @class */ (function () {
    function SliderConstructor(root, options) {
        if (options === void 0) { options = {}; }
        this.root = root;
        this.modelOptions = __assign({}, namespace_1.initialOptions);
        this.viewOptions = __assign({}, namespace_2.initialOptions);
        this.controllerCallbacks = __assign({}, namespace_3.initialOptions);
        this.init(options);
    }
    Object.defineProperty(SliderConstructor.prototype, "data", {
        get: function () {
            return __assign({}, this.model.data, this.view.data);
        },
        enumerable: true,
    ...
    SliderConstructor.prototype.update = function (options) {
        this.setOptions(options);
        this.model.update(this.modelOptions);
        this.view.update(this.viewOptions);
        this.controller.update(this.controllerCallbacks);
    };
    SliderConstructor.prototype.init = function (options) {
        this.model = new model_1.default();
        this.view = new view_1.default(this.root);
        this.setOptions(options);
        this.model.update(this.modelOptions);
        this.view.update(this.viewOptions);
        this.controller = new controller_1.default(this.model, this.view, this.controllerCallbacks);
    };
    SliderConstructor.prototype.setOptions = function (options) {
        this.modelOptions = __assign({}, this.model.data);
        this.viewOptions = __assign({}, this.view.data);
    ...
    SliderConstructor.prototype.setNumberOption = function (optionName, value) {
        if (value !== void (0) && !Number.isNaN(Number(value))) {
            this.modelOptions[optionName] = Number(value);
        }
    };
    SliderConstructor.prototype.setFixedOption = function (optionName, value) {
        if (value !== void (0)) {
            this.modelOptions[optionName] = JSON.parse(value.toString());
            this.viewOptions[optionName] = JSON.parse(value.toString());
        }
    };
    SliderConstructor.prototype.setCallback = function (optionName, value) {
...
var VanillaSlider = /** @class */ (function () {
    function VanillaSlider(node, options) {
        if (options === void 0) { options = {}; }
        var validatedNode = VanillaSlider.getValidateNode(node);
        var slider = new SliderConstructor(validatedNode, options);
        this.setOptions = slider.update.bind(slider);
        this.data = slider.data;
        VanillaSlider.instances.set(validatedNode, this);
    }
    VanillaSlider.getInstance = function (node) {
        var validatedNode = VanillaSlider.getValidateNode(node);
        return VanillaSlider.instances.has(validatedNode) ? VanillaSlider.instances.get(validatedNode) : undefined;
    };
    VanillaSlider.getValidateNode = function (node) {
        var element = typeof node === 'string' ? document.querySelector(node) : node;
        if (this.isDivOrSpan(element)) {
            return element;
        }
    ...
    VanillaSlider.isDivOrSpan = function (node) {
        return (node instanceof HTMLDivElement) || (node instanceof HTMLSpanElement);
    };
    VanillaSlider.instances = new Map();
