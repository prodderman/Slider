...
/******/ 	var hotCurrentHash = "d25a9bdc4f806ed68841"; // eslint-disable-line no-unused-vars
...
var Controller = /** @class */ (function () {
    ...
    Controller.prototype.updateClientsCallbacks = function (callbacks) {
    ...
    Controller.prototype.attachViewEvents = function () {
        ...
        var view = this.view;
        view.events.slide.attach(function (handle, pixels) {
            model.updateHandleValue(handle, _this.convertToReal(pixels));
        });
        view.events.slideStart.attach(function (handle) {
            _this.emitEvent('vanillastart', handle, _this.callbacks.onSlideStart, _this.getModelViewData());
        });
        view.events.slideFinish.attach(function (handle) {
            _this.emitEvent('vanillafinish', handle, _this.callbacks.onSlideFinish, _this.getModelViewData());
        });
    };
    Controller.prototype.attachModelEvents = function () {
        ...
        var model = this.model;
        var view = this.view;
        model.events.stateChanged.attach(function (handle, value) {
            view.changeHandlePosition(handle, _this.convertToPercent(value));
            _this.emitEvent('vanillaslide', handle, _this.callbacks.onSlide, _this.getModelViewData());
        });
    };
    ...
    Controller.prototype.convertToReal = function (pixels) {
        var modelData = this.model.data;
        var sliderSize = this.view.sliderSize;
        var range = modelData.max - modelData.min;
        return this.view.data.orientation === namespace_1.TOrientation.horizontal ?
...
var Model = /** @class */ (function () {
    function Model() {
        this.modelEvents = {
            stateChanged: new observer_1.default(),
        };
        this.options = __assign({}, initial_1.initialOptions);
    }
    Object.defineProperty(Model.prototype, "data", {
        get: function () { return __assign({}, this.options); },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(Model.prototype, "events", {
        get: function () { return __assign({}, this.modelEvents); },
        enumerable: true,
        configurable: true
    });
    Model.prototype.updateState = function (options) {
    ...
    Model.prototype.updateHandleValue = function (handle, rawHandleValue) {
        var opt = this.options;
        var valueFixed = handle === 'from' ? opt.fromFixed : opt.toFixed;
        if (valueFixed) {
            return;
        }
        var minLimitForHandle = handle === 'from' ? opt.min : opt.from;
        var maxLimitForHandle = handle === 'from' ? opt.to : opt.max;
        var valueWithStep = Math.round((rawHandleValue - opt.min) / opt.step);
        var valueOffset = valueWithStep * opt.step + opt.min;
        var valueInDiapason = opt.type ?
        ...
        if (valueInDiapason !== opt[handle]) {
            ...
            this.modelEvents.stateChanged.notify(handle, opt[handle]);
        }
    };
...
var namespace_1 = __webpack_require__("./components/view/namespace.ts");
exports.initialOptions = {
    type: namespace_1.TSliderType.single,
    orientation: namespace_1.TOrientation.horizontal,
    fromFixed: false,
    toFixed: false
};
...
})(TSliderType = exports.TSliderType || (exports.TSliderType = {}));
var TOrientation;
(function (TOrientation) {
    TOrientation["vertical"] = "vertical";
    TOrientation["horizontal"] = "horizontal";
})(TOrientation = exports.TOrientation || (exports.TOrientation = {}));
...
var View = /** @class */ (function () {
    function View(root) {
        ...
        this.viewEvents = {
            ...
            slideFinish: new observer_1.default(),
            slide: new observer_1.default(),
        };
        this.options = __assign({}, initial_1.initialOptions);
        this.init(root);
    }
    ...
    Object.defineProperty(View.prototype, "sliderSize", {
        get: function () { return this.getSliderSize(); },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(View.prototype, "events", {
        get: function () { return __assign({}, this.viewEvents); },
        enumerable: true,
        configurable: true
    });
    View.prototype.updateState = function (viewOptions) {
        this.setOptions(viewOptions);
        this.nodes.track.innerHTML = '';
        this.setNodes();
        this.render();
    };
    View.prototype.emitCustomEvent = function (event, target) {
        this.nodes[target].dispatchEvent(event);
    };
    View.prototype.changeHandlePosition = function (handle, position) {
        var orientation = this.options.orientation;
        var handleNode = this.nodes[handle];
        var base = orientation === namespace_1.TOrientation.vertical ? 'bottom' : 'left';
        handleNode.style[base] = position + "%";
        this.calcRange();
    };
    View.prototype.init = function (root) {
        this.nodes = {
            root: root,
    ...
    View.prototype.render = function () {
        ...
        if (type !== namespace_1.TSliderType.single) {
            nodes.track.appendChild(nodes.range);
        }
        if (type === namespace_1.TSliderType.double) {
    ...
    View.prototype.calcRange = function () {
        if (this.options.type === namespace_1.TSliderType.single) {
            return;
        }
        var opt = this.options;
        var nodes = this.nodes;
        var baseStart = opt.orientation === namespace_1.TOrientation.vertical ? 'bottom' : 'left';
        var baseEnd = opt.orientation === namespace_1.TOrientation.vertical ? 'top' : 'right';
        var handle = opt.type === namespace_1.TSliderType.double ? 'to' : 'from';
        nodes.range.style[baseStart] = opt.type === namespace_1.TSliderType['from-start'] ? '0' : nodes.from.style[baseStart];
        nodes.range.style[baseEnd] = opt.type === namespace_1.TSliderType['from-end'] ? '0' : 100 - parseFloat(nodes[handle].style[baseStart]) + "%";
    };
    ...
    View.prototype.drag = function (event) {
        var coord = this.getRelativeCoord(event);
        if (this.handle) {
            this.viewEvents.slide.notify(this.handle, coord);
        }
    };
    View.prototype.finishDragging = function () {
        if (!this.handle) {
            return;
        }
        window.removeEventListener('mousemove', this.drag);
    ...
    View.prototype.chooseHandle = function (mouseCoord) {
        ...
        if (opt.type !== namespace_1.TSliderType.double) {
            return 'from';
        }
        ...
        if (opt.orientation === namespace_1.TOrientation.horizontal && (mouseCoord < fromCoord)) {
            return 'from';
        }
        if (opt.orientation === namespace_1.TOrientation.horizontal && (mouseCoord > toCoord)) {
            return 'to';
        }
        if (opt.orientation === namespace_1.TOrientation.vertical && (mouseCoord > fromCoord)) {
            return 'from';
        }
        if (opt.orientation === namespace_1.TOrientation.vertical && (mouseCoord < toCoord)) {
            return 'to';
        }
    ...
    View.prototype.getRelativeCoord = function (event) {
        var nodes = this.nodes;
        var axis = this.options.orientation === namespace_1.TOrientation.horizontal ? 'pageX' : 'pageY';
        var offset = this.options.orientation === namespace_1.TOrientation.horizontal ? 'offsetLeft' : 'offsetTop';
        return event[axis] - nodes.track[offset];
    };
    View.prototype.getHandleCoord = function (handle) {
        var offset = this.options.orientation === namespace_1.TOrientation.horizontal ? 'offsetLeft' : 'offsetTop';
        var size = this.options.orientation === namespace_1.TOrientation.horizontal ? 'offsetWidth' : 'offsetHeight';
        return handle[offset] + handle[size] / 2;
    };
    View.prototype.getSliderSize = function () {
        return {
...
(0, _jquery2.default)(document).ready(function () {
  ...
  var indicators = {
    ...
    $finish: (0, _jquery2.default)('.js-indicator-onfinish'),
  ...
  var slider = new _slider2.default('.js-slider', {
    ...
    onSlideStart: function onSlideStart(event, data) {
      controls.$from.val(data.from);
      controls.$to.val(data.to);
    ...
    onSlideFinish: function onSlideFinish(event, data) {
      controls.$from.val(data.from);
      controls.$to.val(data.to);
...
var SliderConstructor = /** @class */ (function () {
    ...
    SliderConstructor.prototype.setOptions = function (options) {
        ...
        if (options.type && options.type in namespace_1.TSliderType) {
            this.viewOptions.type = options.type;
            this.modelOptions.type = options.type === namespace_1.TSliderType.double ? true : false;
        }
        if (options.orientation && options.orientation in namespace_1.TOrientation) {
...
var VanillaSlider = /** @class */ (function () {
    function VanillaSlider(node, options) {
        ...
        if (VanillaSlider.instances.has(validatedNode)) {
            return VanillaSlider.instances.get(validatedNode);
        }
